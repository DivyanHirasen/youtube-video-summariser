...
def transcribe_and_save_video(video_id, channel_id):
    transcript = YouTubeTranscriptApi.get_transcript(video_id)
    text = " ".join([entry['text'] for entry in transcript])

    # Create output directory if it doesn't exist
    output_dir = os.path.join(TRANSCRIPTION_DIR, channel_id, video_id)
...
def main():
    ...
    for video in untranscribed_videos:
        transcribe_and_save_video(video["video_id"], video["channel_id"])
